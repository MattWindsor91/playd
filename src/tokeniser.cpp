...
Tokeniser::Tokeniser() : escape_next{false}, in_word{false}, quote_type{QuoteType::NONE}
{
}
...
void Tokeniser::FeedUnquotedChar(char c)
{
	switch (c) {
		...
		default:
			isspace(c, std::locale::classic()) ? EndWord() : Push(c);
			break;
	}
}
...
{
	Expects(this->escape_next || !(this->quote_type == QuoteType::NONE && isspace(c, std::locale::classic())));
	this->in_word = true;
